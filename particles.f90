...
!   ===== update x-axis for each bin =====
  ...
  do while (associated(part))
    ...
    elseif (part%xp(3) .LT. bot) then
       ...
       if (ireintro .eq. 1 .and. inewpart .eq. 1) then
  ...
  subroutine particle_update_BE(it)
      ...
      do while (associated(part))
        ...
        if (ievap .EQ. 1) then
         ...
         !Andrew Commented this out on Jan 31, 2023. Some sort of error is leading to neg qinf.
